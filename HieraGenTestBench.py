...
# Configure the test run cache counts 配置测试运行缓存计数
L1_cache_count = 2
L2_cache_count = 2

start_time = time.time()

# Change path to the protocol directory path 将路径更改为协议目录路径
os.chdir(os.getcwd() + "/Protocols/")
# Save default path 保存默认路径
path = os.getcwd()
...
for test_case in test_cases:
    ...
    # Parse the input files 分析输入文件
    ParserLL = ProtoParser(file1, test_case[0])
    ParserHL = ProtoParser(file2, test_case[1])

    # Generate cache and directory controllers 生成缓存和目录控制器
    ...
    # Run HieraGen 首先运行HieraGen算法
    # First run HieraGen
    dbg.pheader("HIERAGEN OUTPUT:")
    h_gen = HieraGen(lvl1, lvl2, dbg.dbg)

    # Run ProtoGen for hierachical controller 为分层控制器运行ProtoGen
    config = ProtoConfig()
    ...
    # Then Run ProtoGen for the ll and hl cache and directory controllers 为缓存目录控制器运行ProtoGen算法
    ...
    # Run the Murphi model checker verification  运行Murphi模型检查器验证
